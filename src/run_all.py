...
def main(n_sites: int = 12):
    ...
    summary = dict(
        ...
        격차배율=round(gaps["최대배율"], 1),
        격차상위=f'{gaps["최대격차_상위유형"]} {gaps["최대격차_상위값"]:.1f}',
        격차하위=f'{gaps["최대격차_하위유형"]} {gaps["최대격차_하위값"]:.1f}',
        원도심대비=round(gaps["배율"], 2) if gaps["배율"] == gaps["배율"] else None,
        권역평균=gaps["권역평균"],
        지니계수=round(gaps["지니"], 3),
