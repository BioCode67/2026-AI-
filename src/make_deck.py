...
def build(team=TEAM, members=MEMBERS):
    ...
    tb(sl, 1.15, 3.12, 11.2, .5, "Cheonan Balance Compass", 20, False,
       _hex("#8894A6"))
    if S.get("도메인수", 5) < 3:
        tb(sl, 1.15, 6.36, 10, .3,
           f"※ 현 단계 산출 범위: {S.get('지수범위', '')}", 10, False, _hex("#FAB219"))
...
def _part2(prs, S, cbi, ew, sites, prov, n):
    ...
    # ══ 9. 진단 결과 ═════════════════════════════════════════
    n += 1
    sl = slide(prs, "FINDING · 01", f"살펴본 결과 — {S['생활권수']}개 생활권 {S.get('지수명', 'CBI')}",
               f"최상위 {S['최고']}점, 최하위 {S['최저']}점. "
               f"({S.get('지수범위', '')})")
    ...
    fit(sl, "04_조기경보.png", M, 2.34, 6.5, 4.4)
    e = ew.head(6).copy()
    if len(e):
        ...
        tablette(sl, e, M + 7.0, 2.70, 4.9, colw=[1.9, 1.0, 1.0, 1.0],
                 size=10.5, maxrows=6, rh=.34, note_more=False)
        rect(sl, M + 7.0, 5.02, 4.9, 1.44, LIGHT)
        tb(sl, M + 7.26, 5.22, 4.4, .28, "이 표를 어떻게 보면 되나요", 12.5, True, INK)
        tb(sl, M + 7.26, 5.52, 4.4, .84,
           "· 숫자는 25개 생활권 중 몇 번째로 위험한지를 0~100으로\n"
           "  나타낸 값입니다 (100이 가장 위험).\n"
           "· '변화'가 클수록 지금보다 나빠질 것으로 본 곳입니다.",
           10.5, False, MUTE, line=1.5)
    else:
        rect(sl, M + 7.0, 2.34, 4.9, .9, _hex("#FDF8EC"))
        tb(sl, M + 7.26, 2.52, 4.4, .55,
           "예측 엔진 미실행 — " + S.get("예측미실행사유", ""), 10.5, False, INK, line=1.4)
    footer(sl, n)

    # ══ SHAP — 왜 그렇게 봤는지 ═══════════════════════════════
    if (FIG / "05_SHAP_요인분해.png").exists():
        n += 1
        sl = slide(prs, "FINDING · 04-1", "왜 그렇게 봤는지도 함께 보여드립니다",
                   "숫자만 있으면 '왜 이 동네인가'를 설명하기 어렵습니다. "
                   "어떤 항목이 결과를 끌어올렸는지 나눠서 보여드립니다.")
        fit(sl, "05_SHAP_요인분해.png", M, 2.42, W - 2 * M, 3.5)
        rect(sl, M, 6.06, W - 2 * M, .86, LIGHT)
        tb(sl, M + .3, 6.24, 11.5, .52,
    ...
    rect(sl, M + 6.65, 2.46, 5.25, 2.92, _hex("#EFF7F3"))
    tb(sl, M + 6.95, 2.68, 4.7, .3, "형평성 제약을 넣은 이유", 13.5, True, GREEN)
    tb(sl, M + 6.95, 3.06, 4.7, 2.1,
    ...
    rect(sl, M + 6.65, 5.52, 5.25, 1.06, LIGHT)
    tb(sl, M + 6.95, 5.72, 4.7, .70,
       "결과는 '순위 · 생활권 · 시설유형 · 새로 닿는 인구 · 개선폭'이\n"
